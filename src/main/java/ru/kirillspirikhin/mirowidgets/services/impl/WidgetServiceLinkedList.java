...
public class WidgetServiceLinkedList implements WidgetService {
  ...
  @Override
  public Widget addWidget(final WidgetDescription widgetDescription) {
  ...
  @Override
  public Widget getById(final UUID id) throws WidgetNotFoundException {
    Widget[] local = getAllWidgets().getWidgets();
    for (Widget w : local) {
  ...
  @Override
  public Widget editWidget(final UUID id, final WidgetDescription description)
  ...
  @Override
  public boolean deleteWidget(final UUID id) {
    final ReentrantLock lock = this.reentrantLock;
    lock.lock();
    try {
  ...
  public PagedWidgets getAllWidgets() {
    ...
    return new PagedWidgets(localWidgets,
        localWidgets.length,
        1,
        0,
        localWidgets.length);
  ...
  @Override
  public PagedWidgets getAllWidgets(final Pageable page) {
    Widget[] localWidgets = new Widget[page.getPageSize()];
    int i = 0;
    final int size;
    final ReentrantLock lock = this.reentrantLock;
    lock.lock();
    try {
      size = widgets.size();
      if (page.getOffset() < size) {
        ListIterator<Widget> widgetListIterator = widgets.listIterator((int) page.getOffset());
        for (i = 0; i < page.getPageSize() && widgetListIterator.hasNext(); i++) {
          localWidgets[i] = widgetListIterator.next();
        }
      }
    } finally {
      lock.unlock();
    }
    localWidgets = Arrays.copyOf(localWidgets, i);
    int addPage = size % page.getPageSize() == 0 ? 0 : 1;
    return new PagedWidgets(localWidgets,
        size,
        size / page.getPageSize() + addPage,
        page.getPageNumber(),
        page.getPageSize());
  }
